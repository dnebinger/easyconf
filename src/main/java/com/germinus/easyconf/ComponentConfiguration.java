...
public class ComponentConfiguration {
	...
	protected Map getConfObjectsCache() {
		return confObjectsCache;
	}
    /**
	...
	public Object getConfigurationObject(String confName) {
		if (getConfObjectsCache().get(confName) == null) {
	        try {
	            ConfigurationObjectCache confObjectCache = getConfigurationManager().
	                    readConfigurationObject(getCompanyId(), getComponentName(),
	                                            confName,
				...
				getConfObjectsCache().put(confName, confObjectCache);
	        } catch (IOException e) {
				throw new ConfigurationException(getComponentName(), "Error reading object configuration", e);
	        } catch (SAXException e) {
				throw new ConfigurationException(getComponentName(), "Error parsing the XML file", e);
	        }
        }
        ConfigurationObjectCache confObject = (ConfigurationObjectCache) 
			getConfObjectsCache().get(confName); 
	...
	public void setConfigurationManager(final ConfigurationLoader manager) {
		confManager = manager;
	}
	...
	protected ComponentProperties getAvailableProperties() {
        if (properties != null) {
            return properties;
        }
